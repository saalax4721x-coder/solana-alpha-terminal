...
app.get("/api/whales", async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT address, label, balance_sol, realized_pnl_sol, wins, losses, trades, last_seen_at
     FROM wallets ORDER BY balance_sol DESC NULLS LAST, last_seen_at DESC LIMIT $1 OFFSET $2`,
    [limit, offset]
  );
  res.json({ data: result.rows, page: Math.floor(offset / limit) + 1, limit, total: result.rowCount });
});
...
app.get("/api/whales/activity", async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT a.signature, a.wallet_address, a.mint, t.symbol, t.name, t.image_url,
            a.action, a.token_amount, a.sol_amount, a.timestamp
     FROM whale_activity a LEFT JOIN tokens t ON t.mint=a.mint
...
app.get("/api/whales/today", async (_req, res) => {
  ...
  const result = await pool.query(
    `SELECT wallet_address,
            COUNT(*)::int AS trades,
            COALESCE(SUM(CASE WHEN action='BUY' THEN sol_amount ELSE 0 END),0) AS buy_sol,
            COALESCE(SUM(CASE WHEN action='SELL' THEN sol_amount ELSE 0 END),0) AS sell_sol,
            COALESCE(SUM(CASE WHEN action='SELL' THEN sol_amount ELSE -sol_amount END),0) AS net_flow_sol
     FROM whale_activity WHERE timestamp >= CURRENT_DATE
     GROUP BY wallet_address ORDER BY net_flow_sol DESC LIMIT 200`
  );
...
app.get("/api/tokens", async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT mint, symbol, name, image_url, decimals, market_cap_usd, price_usd,
            liquidity_usd, volume_24h_usd, first_seen_at, last_seen_at
     FROM tokens
     WHERE ($1::numeric IS NULL OR market_cap_usd IS NULL OR market_cap_usd <= $1)
...
app.get("/api/smart-wallets", async (_req, res) => {
  if (!pool) return res.status(503).json({ data: [] });
  const result = await pool.query(
    `SELECT address, balance_sol, realized_pnl_sol, wins, losses, trades,
            CASE WHEN trades > 0 THEN ROUND((wins::numeric / trades::numeric) * 100, 2) ELSE 0 END AS win_rate
     FROM wallets ORDER BY realized_pnl_sol DESC NULLS LAST, win_rate DESC LIMIT 200`
  );
  res.json({ data: result.rows });
});
...
app.listen(port, async () => {
  ...
  if (pool) {
    try {
      await ensureSchema();
      startIngestion(pool);
      console.log("Live Solana ingestion engine started");
    } catch (error) {
